<div wire:poll.2s class="kitchen-tv">
    ...
    @else
        ...
        <div
            wire:key="pagination-wrapper"
            @if($pages->count() > 1)
                x-data="{ page: 0, total: {{ $pages->count() }} }"
                x-init="setInterval(() => { page = (page + 1) % total }, 7000)"
            @else
                x-data="{ page: 0, total: 1 }"
            @endif
        >
            ...
            @if($pages->count() > 1)
                <div class="page-dots">
                    @foreach($pages as $i => $page)
                        <span class="dot" wire:key="dot-{{ $i }}" :class="{ active: page === {{ $i }} }"></span>
                    @endforeach
                </div>
            @endif
